...
public class PriorityScheduler extends Scheduler {
	...
	protected class PriorityQueue extends ThreadQueue {
		...
		protected ThreadState pickNextThread() {
			...
			for (KThread kthread : waitQ){
				if (next == null || getEffectivePriority(kthread) > topPriority ){
					next = kthread;
					topPriority = getEffectivePriority(kthread);
				}
			}
		...
		LinkedList <KThread> waitQ = new LinkedList <KThread>();
	}
		...
		public int getEffectivePriority() {
			...
			for ( PriorityQueue queue : donationQueue){
				if (queue.transferPriority){
					if (queue.transferPriority){
						for (KThread thread : queue.waitQ) {
							int p = getThreadState(thread).getPriority();
